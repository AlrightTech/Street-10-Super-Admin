...
export default function ProductActiveDetail() {
  ...
  useEffect(() => {
    if (id) {
      const found = MOCK_PRODUCTS.find((p) => p.id === id && p.status === 'active')
      if (found) {
        setProduct(found)
      } else {
        // Fallback to default active data
  ...
  const productData = {
    id: product.id,
    product: product.product || 'Flash Sale',
    vendor: product.vendor || 'Vendor',
    category: product.category || 'Promo',
    startDate: product.startDate || 'Immediate',
    endDate: product.endDate || 'Immediate',
