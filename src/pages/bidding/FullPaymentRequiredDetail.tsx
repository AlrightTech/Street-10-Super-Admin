...
/**
 * Full Payment Required Product Detail page
 */
export default function FullPaymentRequiredDetail({ product, auction, mediaUrls, onClose: _onClose }: PaymentRequestedDetailProps) {
            ...
            <div className="border border-gray-200 rounded-lg p-4 sm:p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#F7931E] text-white rounded-lg p-4">
                  <p className="text-sm font-medium mb-1">Auction Ended - Full Payment Required</p>
                  <p className="text-xs text-gray-200">
                    {auction?.endAt ? `Ended: ${formatDate(auction.endAt)}` : 'Full Payment Required'}
                  </p>
                </div>
                ...
                <button 
                  onClick={async () => {
                    const orderId = (auction as any)?.order?.id
                    if (!orderId) {
                      alert('Order not found for this auction')
                      return
                    }
                    if (!window.confirm('Are you sure you want to mark the full payment as paid? This will apply the deposit and mark the order as fully paid.')) {
                      return
                    }
                    try {
                      setMarkingPaid(true)
                      await ordersApi.markFullPaymentPaid(orderId)
                      alert('Full payment marked as paid successfully')
                      // Redirect to bidding products table since status has changed
                      navigate('/building-products')
                    } catch (error: any) {
                      console.error('Error marking full payment as paid:', error)
                      alert(error.response?.data?.message || 'Failed to mark full payment as paid')
                    } finally {
                      setMarkingPaid(false)
                    }
                  }}
                  disabled={markingPaid}
                  className="w-full inline-flex items-center justify-center rounded-lg bg-[#F7931E] px-4 py-2.5 text-sm font-medium text-white transition hover:bg-[#E8840D] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {markingPaid ? 'Marking...' : 'Mark Full Payment Paid'}
                </button>
              </div>
            </div>
