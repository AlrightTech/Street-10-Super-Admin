...
/**
 * Down Payment Required Product Detail page
 */
export default function DownPaymentRequiredDetail({ product, auction, mediaUrls, onClose: _onClose }: PaymentRequestedDetailProps) {
            ...
            <div className="border border-gray-200 rounded-lg p-4 sm:p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#F7931E] text-white rounded-lg p-4">
                  <p className="text-sm font-medium mb-1">Auction Ended - Down Payment Required</p>
                  <p className="text-xs text-gray-200">
                    {auction?.endAt ? `Ended: ${formatDate(auction.endAt)}` : 'Down Payment Required'}
                  </p>
                </div>
                ...
                <button 
                  onClick={async () => {
                    ...
                    if (!window.confirm('Are you sure you want to mark the down payment as paid?')) {
                      return
                    }
                    try {
                      setMarkingPaid(true)
                      await ordersApi.markDownPaymentPaid(orderId)
                      alert('Down payment marked as paid successfully')
                      // Redirect to bidding products table since status has changed
                      navigate('/building-products')
                    } catch (error: any) {
                      console.error('Error marking down payment as paid:', error)
                      alert(error.response?.data?.message || 'Failed to mark down payment as paid')
                    } finally {
                ...
                >
                  {markingPaid ? 'Marking...' : 'Mark Down Payment Paid'}
                </button>
              </div>
            </div>
