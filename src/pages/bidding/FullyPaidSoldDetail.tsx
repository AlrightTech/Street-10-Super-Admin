 ...
 */
export default function FullyPaidSoldDetail({ product, auction, mediaUrls, onClose: _onClose }: FullyPaidSoldDetailProps) {
  ...
  useEffect(() => {
    console.log('📋 FullyPaidSoldDetail - Auction prop:', auction)
    console.log('📋 FullyPaidSoldDetail - Product prop:', product)
  ...
  useEffect(() => {
    ...
    if (auctionId) {
      const fetchBids = async () => {
        try {
          setLoadingBids(true)
          console.log('🔍 Fetching bids for fully-paid-sold auction:', auctionId)
  ...
  // Get winning bid (highest bid marked as winning)
  const winningBid = bids.find(bid => bid.isWinning) || (bids.length > 0 ? bids.reduce((highest, bid) => 
    parseFloat(bid.amountMinor) > parseFloat(highest.amountMinor) ? bid : highest
  ) : null)
  const currentHighestBid = winningBid ? formatPrice(winningBid.amountMinor, fullAuction?.product?.currency || 'QAR') : 'No bids'
            ...
            <div className="border border-gray-200 rounded-lg p-4 sm:p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-100 rounded-lg p-4">
                  <p className="text-sm font-medium text-gray-700 mb-1">Auction Ended - Fully Paid</p>
