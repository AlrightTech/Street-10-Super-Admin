...
const MOCK_PUSH_NOTIFICATIONS: (PushNotification & {
  ...
  },
  {
    ...
    fullMessage: 'System maintenance notification',
    startDate: 'Dec 22, 2024',
    endDate: 'Dec 22, 2024',
    sendTime: '10:00 AM',
    recipients: '8,100 recipients',
    url: 'https://www.example-site123.com/maintenance',
  },
]

export default function PushNotificationSendDetail() {
  ...
  useEffect(() => {
    if (id) {
      const found = MOCK_PUSH_NOTIFICATIONS.find((n) => n.id === id)
      if (found) {
        setNotification(found)
      } else {
        // Fallback to default data
        setNotification(MOCK_PUSH_NOTIFICATIONS[0])
      }
    }
  }, [id])
  ...
  const notificationData = {
    ...
    phone: notification.phone || '+1 234 567 8900',
    fullMessage: notification.fullMessage || 'Flash Sale Alert! Get 50% off an all electronics for the next 24 hours. Don\'t miss out on this incredible deal - shop now before it\'s too late! Use code: FLASH50',
    ...
    url: notification.url || 'https://www.example-site123.com/deals/flash-sole',
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Main Navigation Bar */}
      <nav className="flex flex-wrap items-center gap-1 sm:gap-2 md:gap-4
       border-b border-gray-200 overflow-x-auto pt-3 pb-1">
        <button
          type="button"
