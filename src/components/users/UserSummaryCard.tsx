 ...
 */
export default function UserSummaryCard({ user, onEdit, onViewKYC }: UserSummaryCardProps) {
  const navigate = useNavigate()

  const handleEdit = () => {
    if (onEdit) onEdit()
    else navigate(`/users/${user.id}/edit`)
  }

  const handleViewKYC = () => {
    if (onViewKYC) onViewKYC()
    else navigate(`/view-kyc/${user.id}`)
  }
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm overflow-hidden relative">
      {/* View KYC - top right corner */}
      <div className="absolute top-4 right-4">
        <button
          type="button"
          onClick={handleViewKYC}
          className="rounded-lg bg-[#F39C12] px-4 py-2 text-sm font-medium text-white hover:bg-[#E67E22] transition-colors whitespace-nowrap"
        >
          View KYC
        </button>
      </div>
      ...
      <div className="absolute bottom-4 right-4">
        <button
          type="button"
          onClick={handleEdit}
          className="flex items-center justify-center gap-2 rounded-lg bg-[#F39C12] px-4 py-2 text-sm font-medium text-white hover:bg-[#E67E22] transition-colors whitespace-nowrap"
        >
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 xl:gap-12 items-center pr-32 sm:pr-36">
        ...
        <div className="flex flex-col gap-6 w-full lg:w-auto flex-1">
          <div>
          ...
          <div>
            ...
            <p className="text-sm font-normal text-gray-600 mb-3">QAR {(user.walletBalance || 0).toLocaleString()}</p>
