...
export default function FinanceTable({ transactions, onActionSelect, onRowClick, emptyState }: FinanceTableProps) {
  if (transactions.length === 0) {
    return (
      <div className="flex min-h-[240px] flex-col
       items-center justify-center
        rounded-2xl border border-dashed border-gray-200 bg-gray-50 py-12 text-center">
        {emptyState ?? (
          <>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-700 text-xs
                       sm:text-sm truncate max-w-[100px] sm:max-w-none">{transaction.user}</span>
                    </div>
                  </TableCell>
  ...
  return (
    <th
      ...
      className={`whitespace-nowrap py-2 
        sm:py-2 text-xs sm:text-sm font-semibold 
        tracking-wide text-gray-700 border-b border-gray-300 bg-white ${textAlign} first:pl-4 sm:first:pl-6 last:pr-4 sm:last:pr-6`}
    >
      {children}
    </th>
  )
